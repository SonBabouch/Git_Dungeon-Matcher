...
public class CombatProfilListTuto : MonoBehaviour
{
    ...
    public void UpdateVisualMatch()
    {
        ...
        monsterContainer = MenuManagerTuto.Instance.matchManager.matchList[MenuManagerTuto.Instance.matchManager.matchList.Count - 1];
    ...
    public IEnumerator UpdateVisualEndCombat()
    {
        ...
        if (MenuManager.Instance.listManager.currentTest < 1)
        {
            ...
            bonusText.GetComponent<TextMeshProUGUI>().text = "BONUS :  " + ((MenuManagerTuto.Instance.listManager.currentTest - 1) * 5).ToString();

            for (int i = 0; i < (MenuManagerTuto.Instance.listManager.currentTest - 1) * 3; i++)
            {
                chanceClaim++;
        ...
        else
        {
            ...
            if (isClaim)
            {
                ...
                MenuManagerTuto.Instance.monsterEncyclopedie.GetNewMonsters();
                MenuManagerTuto.Instance.canvasManager.bagCanvas.SortBag();
                PlayerLevelTuto.currentExperienceTuto += 7;
                ...
                MenuManagerTuto.Instance.canvasManager.listCanvas.popButton.SetActive(true);
            }
            else
            {
    ...
    {
        MenuManagerTuto.Instance.listManager.currentTest++;
        chanceDrop.enabled = false;
