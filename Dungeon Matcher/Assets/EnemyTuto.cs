...
{
    public static EnemyTuto Instance;
    public float health;
    ...
    private void Update()
    {
        ...
        {
            SkillFeedbackTuto.Instance.EnemiDefenseFeedback();
            SkillFeedbackTuto.Instance.EnemiAccelerateDecelerateFeedback();
        }
    }
    ...
    public IEnumerator EnemyChargeAttack(SkillTuto skillToCharge)
    {
        ...
        ConversationManagerTuto.Instance.SendMessagesEnemy(skillToCharge, 0);
        yield return null;
    }
    ...
    public IEnumerator EndEnemyChargeAttack(SkillTuto skillToCharge)
    {
        ...
        if (ConversationManagerTuto.Instance.enemyChargingAttack.activeInHierarchy)
        {
            ConversationManagerTuto.Instance.UpdateLastMessageState(skillToCharge);
        }
        if (!skillToCharge.comesFromCurse)
        {
            if (!CombatManagerTuto.Instance.isCombatEnded && !PlayerTuto.Instance.playerUsedBreak)
            {
        ...
        PlayerTuto.Instance.playerUsedBreak = false;
